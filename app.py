...
function calcPayment(rate, copay, elig, attd, adj) {
  ...
  const days = (adj !== null && adj !== '' && adj !== undefined)
    ? parseFloat(adj)
    : (parseFloat(elig) || 0);
...
FORM LAYOUT — each child record block looks like this:
  ...
  FIRSTNAME LASTNAME         <- CLIENT'S NAME — this is the PARENT
    CHILD NAME               <- CHILD'S NAME — this is the child
    MM/DD/YYYY               <- child date of birth
    46.00  F  1.00  ...      <- rate rows (F, P, S)

PARENT vs CHILD NAME — read carefully:
- The form has TWO columns of names on the LEFT side:
    Left column header:  "CHILD CARE CASE NO. / CLIENT'S NAME"
    Right column header: "CHILD'S NAME / DATE OF BIRTH"
- parent_client: found in the LEFT column, on the line BELOW "IVR CASE#:...".
  It is always an ALL-CAPS adult name hugging the LEFT MARGIN of the page.
  Example block:
    13698 53045 60008        <- case number, ignore
    IVR CASE#:3-200-113033  <- IVR line, ignore
    DAVONTE BARNES           <- THIS is parent_client
- child_name: found in the RIGHT/CENTER column, aligned with the child's date of birth.
  It appears to the RIGHT of the case number block, in the "CHILD'S NAME" column.
  Example: "SHILOH BARNES" appearing next to "8/02/2025" is the child.
- NEVER swap these — the parent is always on the left with the case number, the child is on the right.
- NEVER output column header text as data. The following are column headers, not names:
  "CHILD'S NAME", "DATE OF BIRTH", "CHILD'S NAME DATE OF BIRTH",
  "CLIENT'S NAME", "CHILD CARE CASE NO.", "RATE", "SCH AGE", "CO-PAY"
  If you see any of these as a field value, you have read a header row — skip it entirely.
...
- A row is "filled" ONLY if it has BOTH a rate AND an actual number in the DAYS ELIG column (Col 5).
- The presence of a printed rate (e.g. 34.00, 27.00, 23.00) alone does NOT make a row filled.
- If a row has a rate but DAYS ELIG is blank (just a blank line ——) → skip that row entirely.
- Only output a record for a row when elig_days is an actual number.
- USUALLY only one row is filled → output ONE record for that child.
- BUT if TWO OR MORE rows are filled (each has both a rate AND elig_days), output ONE RECORD PER FILLED ROW,
  each with the same parent_client, child_name, and dob, but with that row's own rate, copay, elig_days, attd_days, and adj_days.
...
- children: only records with actual data (name + rate + elig_days). Skip column header rows.
...
def run_job(job_id, file_bytes, filename):
    try:
        ...
        for i, page in enumerate(pages):
            ...
            # Filter out rows with no elig_days and any that contain header text
            header_strings = {
                "child's name", "date of birth", "child's name date of birth",
                "client's name", "child care case no.", "rate", "sch age", "co-pay"
            }
            def is_valid(c):
                if not c.get('child_name') or c.get('elig_days') is None:
                    return False
                if c.get('child_name', '').strip().lower() in header_strings:
                    return False
                if c.get('parent_client', '').strip().lower() in header_strings:
                    return False
                return True

            kids = [c for c in (result.get('children') or []) if is_valid(c)]
...
def build_excel(provider_info, children, summary):
    ...
    for ri, child in enumerate(children, start=hr+1):
        ...
        # Column 8 (H): Payment — if Adj Days (G) filled use G, else use Elig Days (E)
        pay = ws.cell(ri, 8)
