    ...
    for i in range(len(data['text'])):
        ...
        if txt and int(data['conf'][i]) > 0:
            result.append({
                ...
                'width': data['width'][i], 'height': data['height'][i], 'conf': int(data['conf'][i]),
            })
    return result
...
# Column x-ranges as fractions of page width (calibrated to IL444-3492A at 300dpi)
COL_FRACS = {
    'rate':  (0.43, 0.52),
    'sch':   (0.52, 0.58),
    'copay': (0.535, 0.665),
...
def parse_certificate_pdf(pdf_bytes: bytes) -> dict:
    ...
    all_words = [ocr_words(p) for p in pages]

    # ── Provider info (pages 1-2 full text) ──────────────────────────────────
    for pg_idx in range(min(3, len(pages))):
        txt = pytesseract.image_to_string(pages[pg_idx])
    ...
    # Page 1: provider name may appear standalone (no "PROVIDER NAME:" prefix)
    if not provider_info.get('name'):
    ...
    # Signature/summary page (last page)
    txt_last = pytesseract.image_to_string(pages[-1])
    for line in txt_last.splitlines():
        line = line.strip()
    ...
    for page_idx, words in enumerate(all_words[:-1]):
        ...
        # Find case-number anchor words: 5-digit numbers near left edge
        case_words = [w for w in words
                      if re.match(r'^\d{5}$', w['text']) and w['left'] < 0.18 * page_w]

        # Group into rows (same y ± 30px) → each group = one case entry
        used = set()
        case_rows = []
        for i, w in enumerate(case_words):
            ...
            if len(group) >= 2:
                ...
                case_no = ' '.join(x['text'] for x in sorted(group, key=lambda x: x['left']))
                case_rows.append((y_c, case_no))

        for ci, (y_case, case_no) in enumerate(case_rows):
            y_end = case_rows[ci + 1][0] if ci + 1 < len(case_rows) else y_case + 700

            row_words = [w for w in words
                         if (w['top'] + w['height']/2) > y_case - 30
                         and (w['top'] + w['height']/2) < y_end]

            # Child name: same y-band as case, x > 20% of page, all-caps alpha
            ...
            # DOB: date pattern, same x-band, below case row
            ...
            # Client name: left edge (x < 20%), below case row, before next case
            ...
            for w in row_words:
                ...
                if xlo_sch <= cx <= xhi_sch and w['text'].upper() in ('F', 'E', '='):
                    ...
                    break
            # Fallback: first rate-like number in rate column
            if f_row_y is None:
                xlo_r, xhi_r = xr('rate')
...
if uploaded:
    ...
    with st.spinner("Running OCR — this may take 20–40 seconds…"):
        try:
            result = parse_certificate_pdf(pdf_bytes)
    ...
    for col, (label, val) in zip(cols, [
        ("Provider", pi.get("name","—")), ("Report Period", pi.get("period","—")),
        ("DHS Provider ID", pi.get("dhs_id","—")), ("Date of Issue", pi.get("date_of_issue","—")),
    ]):
        col.markdown(f'<div class="info-card"><h4>{label}</h4><p>{val}</p></div>', unsafe_allow_html=True)
    ...
    if children:
        ...
        st.warning("No records extracted — the PDF scan quality may be too low. "
                   "You can manually add rows after downloading a blank template.")
        children_out = []
