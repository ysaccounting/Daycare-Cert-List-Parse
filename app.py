...
- provider: fill from header if visible; leave fields empty string if not present
...
def parse_page_with_claude(pil_image, page_num=1):
    ...
    try:
        ...
        app.logger.error(f"Page {page_num} JSON parse error: {e}\nRaw response: {raw[:500]}")
...
def get_pages(file_bytes, filename):
    ...
    if ext == '.pdf':
        ...
        app.logger.info(f"PDF converted: {len(pages)} pages")
        return pages
    elif ext in ('.jpg', '.jpeg', '.png'):
        img = Image.open(io.BytesIO(file_bytes)).convert("RGB")
        app.logger.info("Image file loaded as single page")
        return [img]
    else:
...
def parse_upload(file_bytes, filename):
    ...
    for i, page in enumerate(pages):
        ...
        try:
            ...
            app.logger.error(f"Page {i+1} failed: {e}")
            continue  # skip bad pages, keep going

        if not result.get("is_data_page", True):
            app.logger.info(f"Page {i+1}: not a data page, skipping")
            continue
        ...
        # Collect children
        page_children = result.get("children") or []
        for child in page_children:
            if child.get("child_name"):
                all_children.append(child)
        app.logger.info(f"Page {i+1}: {len(page_children)} children found")
    ...
    app.logger.info(f"Parse complete: {len(all_children)} total children, provider={provider_info.get('name')}")
...
def parse():
    ...
    if ext not in ALLOWED_EXTENSIONS:
        return jsonify({"error": f"Unsupported file type '{ext}'. Please upload a PDF, JPG, or PNG."}), 400

    file_bytes = f.read()
    app.logger.info(f"Received file: {f.filename} ({len(file_bytes)} bytes)")

    try:
        provider_info, children, summary = parse_upload(file_bytes, f.filename)
    except Exception as e:
        app.logger.error(f"parse_upload failed: {traceback.format_exc()}")
...
def server_error(e):
    ...
    return jsonify({"error": "Internal server error. Check Railway logs for details."}), 500
