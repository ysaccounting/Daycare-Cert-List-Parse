...
- If a row has a rate but DAYS ELIG is blank (just a blank line ——) → skip that row entirely, do not output a record for it.
...
def run_job(job_id, file_bytes, filename):
    try:
        ...
        for i, page in enumerate(pages):
            ...
            kids = [c for c in (result.get('children') or []) if c.get('child_name') and c.get('elig_days') is not None]
