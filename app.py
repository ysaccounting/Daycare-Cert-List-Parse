...
- Only ONE row will have a number in the DAYS ELIG column — use THAT row's rate and copay.
- rate: dollar amount from the row where DAYS ELIG is filled in
- copay: PARENT CO-PAY from that same row; use 0.0 if blank
- Example: if F row has no DAYS ELIG but P row has DAYS ELIG=22, use the P row rate and copay.
- NEVER default to the F row if elig_days appear on a different row.
