...
let parsedData = null;    // merged result across all files
let isRunning  = false;
...
function addFiles(files) {
  for (const file of files) {
    ...
    if (fileQueue.some(f => f.file.name === file.name && f.file.size === file.size)) continue; // dedupe
...
async function processFile(entry, fileIndex, totalFiles) {
  ...
  return new Promise(resolve => {
    ...
    evtSource.addEventListener('progress', e => {
      ...
      // Overall progress = file progress within total files
      const filePct   = d.pct;
    ...
    evtSource.addEventListener('done', e => {
      ...
      if (d.error) {
        ...
        addLog(`\u2705 ${file.name}: ${d.count} children extracted`);
        // Tag each child with source filename
...
parseBtn.addEventListener('click', async () => {
  ...
  // Mark all pending
  fileQueue.forEach(f => { if (f.status === 'pending') f.status = 'pending'; });

  const results = [];
  const pendingFiles = fileQueue.filter(f => f.status !== 'done'); // allow re-run of failed
  ...
  parsedData = merged;
  progressText.textContent = `Done \u2014 ${merged.children.length} children from ${results.length} file(s).`;
...
function mergeResults(results) {
  // Provider info from first successful result
...
FORM LAYOUT — each child record block looks like this:
  ...
  FIRSTNAME LASTNAME         <- CLIENT'S NAME — this is the PARENT. Use ONLY this name for parent_client.
    ...
    46.00  F  1.00  ...      <- rate rows

The columns left to right for each child's rate rows are:
  RATE  TYPE(F/P/S)  PARENT CO-PAY  ADJUSTED CO-PAY  DAYS ELIG  ADJ ELIG  ATTD  CODE

Column mapping to JSON fields:
- rate:      the dollar amount in the RATE column for whichever row (F, P, or S) has DAYS ELIG filled in
- copay:     the PARENT CO-PAY from that same row; use 0.0 if blank
- elig_days: the "DAYS ELIG" column — typically a printed number like 22
- adj_days:  the "ADJ ELIG" column — handwritten; null if blank (blank line —— means null, not zero)
- attd_days: the "ATTD" column — handwritten; null if blank (blank line —— means null, not zero)

CRITICAL COLUMN RULES:
- There are THREE separate day columns: DAYS ELIG, ADJ ELIG, and ATTD. Read each independently.
- If only ATTD has a handwritten number and ADJ ELIG is blank → attd_days=N, adj_days=null
- If both ADJ ELIG and ATTD have handwritten numbers → set both fields
- If only ADJ ELIG has a handwritten number and ATTD is blank → adj_days=N, attd_days=null
- NEVER put the ATTD value into adj_days or vice versa
- A blank line (——) always means null, never zero

Rules:
- parent_client: the CLIENT'S NAME — the all-caps adult name that appears BELOW the IVR CASE# line.
  NEVER put the case number (digits), the IVR CASE# string, or any number in parent_client.
  Example: if you see "IVR CASE#:3-200-299684" then "KIYATA PERRY" — parent_client = "KIYATA PERRY"
- child_name: the child's name, which appears indented or on the next line after the client name, with a DOB below it
...
- rate: use the rate from whichever row (F, P, or S) actually has elig_days filled in.
  Each child has up to 3 rate rows: F (full-day), P (part-day), S (school-age).
  Only ONE of these rows will have a number in the elig_days column — use THAT row's rate and copay.
  Example: if F=46.00 has no elig_days but P=23.00 has elig_days=22, use rate=23.00.
  Example: if S=23.00 has elig_days=21, use rate=23.00.
  NEVER default to the F row rate if the elig_days appear on a different row.
- copay: use the co-pay from the SAME row that has elig_days filled in; use 0.0 if blank
        ...
        for i, page in enumerate(pages):
            ...
            if kids:
                push_event(job_id, 'progress', {'pct': pct, 'msg': f'Page {i+1} of {n}…',
                    'detail': f'Page {i+1}: {len(kids)} children (total: {len(all_children)})'})
...
def build_excel(provider_info, children, summary):
    ...
    for c, h in enumerate(["Parent/Client","Child's Name",
                            "Rate ($)","Co-Pay ($)","Elig Days","Attd Days","Adj Days","Payment ($)"], 1):
        cell = ws.cell(hr, c, h)
    ...
    for ri, child in enumerate(children, start=hr+1):
        ...
        for ci, val in enumerate([
            ...
            child.get("rate"), child.get("copay"),
            child.get("elig_days"), child.get("attd_days"), child.get("adj_days")], 1):
            cell = ws.cell(ri, ci, val)
            cell.font = Font(name="Arial", size=10); cell.fill = fill
            cell.border = bdr; cell.alignment = Alignment(horizontal="center")
            if ci in (3,4): cell.number_format = '$#,##0.00'
        pay = ws.cell(ri, 9)
        pay.value = f'=IF(G{ri}<>"",C{ri}*G{ri},C{ri}*E{ri})-D{ri}'
    ...
    for c in range(1,9):
        cell = ws.cell(tr, c); cell.border = bdr
    ...
    ws2["A1"].fill = hf; ws2["A1"].alignment = Alignment(horizontal="center", vertical="center")
