  ...
  <div id="results-section">
    <div class="card">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr>
            <th>File</th><th>Parent/Client</th><th>Child's Name</th><th>Date of Birth</th>
...
function renderResults(data) {
  ...
  children.forEach((c, idx) => {
    ...
    tr.innerHTML = `
      <td class="source-badge" style="font-size:0.72rem;color:#7a9ab8;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${src}">${src}</td>
...
def build_excel(provider_info, children, summary):
    ...
    hr = 8
    for c, h in enumerate(["Source File","Parent/Client","Child's Name","Date of Birth",
    ...
    for ri, child in enumerate(children, start=hr+1):
        ...
        pay = ws.cell(ri, 10)
        pay.value = f'=IF(I{ri}<>"",E{ri}*I{ri},E{ri}*G{ri})-F{ri}'
    ...
    ws.cell(tr, 8, f"=SUM(H{hr+1}:H{hr+len(children)})")
    ws.cell(tr, 10, f"=SUM(J{hr+1}:J{hr+len(children)})").number_format = '$#,##0.00'
    for i, w in enumerate([20,24,20,14,11,11,10,10,10,13],1):
